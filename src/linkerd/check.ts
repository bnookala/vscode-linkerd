import { CheckStage } from './provider';
...
export class CheckController {
    check = (stage: string): LinkerdCheckCLIOutput => {
        let checkParam = '';
     ...
     */
   structuredCheckOutput = (checkOutput: LinkerdCheckCLIOutput): LinkerdCheck => {
        ...
        for (let index = 0; index < lines.length; index++) {
            ...
            if (strippedOutputLine.startsWith(SUCCESS_SYMBOL)) {
                linkerdOutputSections[currentSection].push(
                    this.checkStatusObject(strippedOutputLine)
                );
                continue;
            }
            ...
            if (strippedOutputLine.startsWith(FAIL_SYMBOL)) {
                const failedCheck = this.checkStatusObject(strippedOutputLine);
                const failedCheckHints = this.failedStatusHints(index, lines);
    ...
    checkStatusObject = (line: string): LinkerdCheckCondition => {
        return {
    ...
    failedStatusHints = (index: number, lines: string[]) => {
